 ...
 * _AINAKAN_ENABLE_PYDEBUG.
 */
#if defined (_DEBUG) && !defined (_AINAKAN_ENABLE_PYDEBUG)
# undef _DEBUG
# include <pyconfig.h>
# define _DEBUG
#else
...
#define PYAINAKAN_TYPE(name) \
  (&_PYAINAKAN_TYPE_VAR (name, type))
#define PYAINAKAN_TYPE_OBJECT(name) \
  PYAINAKAN_TYPE (name)->object
#define _PYAINAKAN_TYPE_VAR(name, var) \
  G_PASTE (G_PASTE (G_PASTE (Py, name), _), var)
#define PYAINAKAN_DEFINE_BASETYPE(pyname, cname, init_func, destroy_func, ...) \
  _PYAINAKAN_DEFINE_TYPE_SLOTS (cname, __VA_ARGS__); \
  _PYAINAKAN_DEFINE_TYPE_SPEC (cname, pyname, Py_TPFLAGS_DEFAULT | Py_TPFLAGS_BASETYPE); \
  static PyGObjectType _PYAINAKAN_TYPE_VAR (cname, type) = \
  { \
    .parent = NULL, \
    .object = NULL, \
...
#define PYAINAKAN_DEFINE_TYPE(pyname, cname, parent_cname, init_func, destroy_func, ...) \
  _PYAINAKAN_DEFINE_TYPE_SLOTS (cname, __VA_ARGS__); \
  _PYAINAKAN_DEFINE_TYPE_SPEC (cname, pyname, Py_TPFLAGS_DEFAULT); \
  static PyGObjectType _PYAINAKAN_TYPE_VAR (cname, type) = \
  { \
    .parent = PYAINAKAN_TYPE (parent_cname), \
    .object = NULL, \
    .init_from_handle = (PyGObjectInitFromHandleFunc) init_func, \
    .destroy = destroy_func, \
  }
#define PYAINAKAN_REGISTER_TYPE(cname, gtype) \
  G_BEGIN_DECLS \
  { \
    PyGObjectType * t = PYAINAKAN_TYPE (cname); \
    t->object = PyType_FromSpecWithBases (&_PYAINAKAN_TYPE_VAR (cname, spec), \
  ...
  G_END_DECLS
#define _PYAINAKAN_DEFINE_TYPE_SPEC(cname, pyname, type_flags) \
  static PyType_Spec _PYAINAKAN_TYPE_VAR (cname, spec) = \
  { \
    .name = pyname, \
    .basicsize = sizeof (G_PASTE (Py, cname)), \
    .itemsize = 0, \
    .flags = type_flags, \
    .slots = _PYAINAKAN_TYPE_VAR (cname, slots), \
  }
#define _PYAINAKAN_DEFINE_TYPE_SLOTS(cname, ...) \
  static PyType_Slot _PYAINAKAN_TYPE_VAR (cname, slots)[] = \
  { \
    __VA_ARGS__ \
    { 0 }, \
  }
...
static struct PyModuleDef PyAinakan_moduledef = { PyModuleDef_HEAD_INIT, "_ainakan", "Ainakan", -1, NULL, };
...
static GHashTable * ainakan_exception_by_error_code;
...
#define AINAKAN_TYPE_PYTHON_AUTHENTICATION_SERVICE (ainakan_python_authentication_service_get_type ())
G_DECLARE_FINAL_TYPE (AinakanPythonAuthenticationService, ainakan_python_authentication_service, AINAKAN, PYTHON_AUTHENTICATION_SERVICE, GObject)
...
struct _AinakanPythonAuthenticationService
{
  GObject parent;
  PyObject * callback;
  GThreadPool * pool;
...
static gboolean PyDeviceManager_is_matching_device (AinakanDevice * device, PyObject * predicate);
...
static AinakanRemoteDeviceOptions * PyDeviceManager_parse_remote_device_options (const gchar * certificate_value, const gchar * origin,
    const gchar * token, gint keepalive_interval);

static PyObject * PyDevice_new_take_handle (AinakanDevice * handle);
static int PyDevice_init (PyDevice * self, PyObject * args, PyObject * kw);
static void PyDevice_init_from_handle (PyDevice * self, AinakanDevice * handle);
...
static AinakanApplicationQueryOptions * PyDevice_parse_application_query_options (PyObject * identifiers_value, const gchar * scope_value);
static PyObject * PyDevice_enumerate_processes (PyDevice * self, PyObject * args, PyObject * kw);
static AinakanProcessQueryOptions * PyDevice_parse_process_query_options (PyObject * pids_value, const gchar * scope_value);
...
static AinakanSessionOptions * PyDevice_parse_session_options (const gchar * realm_value, guint persist_timeout);
...
static PyObject * PyApplication_new_take_handle (AinakanApplication * handle);
static int PyApplication_init (PyApplication * self, PyObject * args, PyObject * kw);
static void PyApplication_init_from_handle (PyApplication * self, AinakanApplication * handle);
...
static PyObject * PyProcess_new_take_handle (AinakanProcess * handle);
static int PyProcess_init (PyProcess * self, PyObject * args, PyObject * kw);
static void PyProcess_init_from_handle (PyProcess * self, AinakanProcess * handle);
...
static PyObject * PySpawn_new_take_handle (AinakanSpawn * handle);
static int PySpawn_init (PySpawn * self, PyObject * args, PyObject * kw);
static void PySpawn_init_from_handle (PySpawn * self, AinakanSpawn * handle);
static void PySpawn_dealloc (PySpawn * self);
static PyObject * PySpawn_repr (PySpawn * self);

static PyObject * PyChild_new_take_handle (AinakanChild * handle);
static int PyChild_init (PyChild * self, PyObject * args, PyObject * kw);
static void PyChild_init_from_handle (PyChild * self, AinakanChild * handle);
...
static PyObject * PySession_new_take_handle (AinakanSession * handle);
static int PySession_init (PySession * self, PyObject * args, PyObject * kw);
static void PySession_init_from_handle (PySession * self, AinakanSession * handle);
...
static AinakanScriptOptions * PySession_parse_script_options (const gchar * name, gconstpointer snapshot_data, gsize snapshot_size,
    const gchar * runtime_value);
static PyObject * PySession_snapshot_script (PySession * self, PyObject * args, PyObject * kw);
static AinakanSnapshotOptions * PySession_parse_snapshot_options (const gchar * warmup_script, const gchar * runtime_value);
static PyObject * PySession_setup_peer_connection (PySession * self, PyObject * args, PyObject * kw);
static AinakanPeerOptions * PySession_parse_peer_options (const gchar * stun_server, PyObject * relays);
static PyObject * PySession_join_portal (PySession * self, PyObject * args, PyObject * kw);
static AinakanPortalOptions * PySession_parse_portal_options (const gchar * certificate_value, const gchar * token, PyObject * acl_value);

static PyObject * PyScript_new_take_handle (AinakanScript * handle);
...
static void PyRelay_init_from_handle (PyRelay * self, AinakanRelay * handle);
static void PyRelay_dealloc (PyRelay * self);
static PyObject * PyRelay_repr (PyRelay * self);

static PyObject * PyPortalMembership_new_take_handle (AinakanPortalMembership * handle);
...
static void PyPortalService_init_from_handle (PyPortalService * self, AinakanPortalService * handle);
...
static AinakanPythonAuthenticationService * ainakan_python_authentication_service_new (PyObject * callback);
static void ainakan_python_authentication_service_iface_init (gpointer g_iface, gpointer iface_data);
static void ainakan_python_authentication_service_dispose (GObject * object);
static void ainakan_python_authentication_service_authenticate (AinakanAuthenticationService * service, const gchar * token,
    GCancellable * cancellable, GAsyncReadyCallback callback, gpointer user_data);
static gchar * ainakan_python_authentication_service_authenticate_finish (AinakanAuthenticationService * service, GAsyncResult * result,
    GError ** error);
static void ainakan_python_authentication_service_do_authenticate (GTask * task, AinakanPythonAuthenticationService * self);
...
static gboolean PyCompiler_set_options (AinakanCompilerOptions * options, const gchar * project_root_value, const gchar * output_format_value,
...
static AinakanPackageInstallOptions * PyPackageManager_parse_install_options (const gchar * project_root, const char * role_value,
    PyObject * specs_value, PyObject * omits_value);

static PyObject * PyPackage_new_take_handle (AinakanPackage * handle);
static int PyPackage_init (PyPackage * self, PyObject * args, PyObject * kw);
static void PyPackage_init_from_handle (PyPackage * self, AinakanPackage * handle);
static void PyPackage_dealloc (PyPackage * self);
static PyObject * PyPackage_repr (PyPackage * self);

static PyObject * PyPackageSearchResult_new_take_handle (AinakanPackageSearchResult * handle);
static int PyPackageSearchResult_init (PyPackageSearchResult * self, PyObject * args, PyObject * kw);
static void PyPackageSearchResult_init_from_handle (PyPackageSearchResult * self, AinakanPackageSearchResult * handle);
...
static PyObject * PyPackageInstallResult_new_take_handle (AinakanPackageInstallResult * handle);
static int PyPackageInstallResult_init (PyPackageInstallResult * self, PyObject * args, PyObject * kw);
static void PyPackageInstallResult_init_from_handle (PyPackageInstallResult * self, AinakanPackageInstallResult * handle);
...
static PyObject * PyAinakan_raise (GError * error);
static gchar * PyAinakan_repr (PyObject * obj);
static guint PyAinakan_get_max_argument_count (PyObject * callable);
...
PYAINAKAN_DEFINE_BASETYPE ("_ainakan.Object", GObject, NULL, g_object_unref,
  { Py_tp_doc, "Ainakan Object" },
  { Py_tp_init, PyGObject_init },
  { Py_tp_dealloc, PyGObject_dealloc },
  { Py_tp_methods, PyGObject_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.DeviceManager", DeviceManager, GObject, NULL, ainakan_unref,
  { Py_tp_doc, "Ainakan Device Manager" },
  { Py_tp_init, PyDeviceManager_init },
  { Py_tp_dealloc, PyDeviceManager_dealloc },
  { Py_tp_methods, PyDeviceManager_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.Device", Device, GObject, PyDevice_init_from_handle, ainakan_unref,
  { Py_tp_doc, "Ainakan Device" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Application", Application, GObject, PyApplication_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Application" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Process", Process, GObject, PyProcess_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Process" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Spawn", Spawn, GObject, PySpawn_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Spawn" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Child", Child, GObject, PyChild_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Child" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Crash", Crash, GObject, PyCrash_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Crash Details" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Bus", Bus, GObject, NULL, g_object_unref,
  { Py_tp_doc, "Ainakan Message Bus" },
  { Py_tp_methods, PyBus_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.Service", Service, GObject, NULL, g_object_unref,
  { Py_tp_doc, "Ainakan Service" },
  { Py_tp_methods, PyService_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.Session", Session, GObject, PySession_init_from_handle, ainakan_unref,
  { Py_tp_doc, "Ainakan Session" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Script", Script, GObject, NULL, ainakan_unref,
  { Py_tp_doc, "Ainakan Script" },
  { Py_tp_methods, PyScript_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.Relay", Relay, GObject, PyRelay_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Relay" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.PortalMembership", PortalMembership, GObject, NULL, ainakan_unref,
  { Py_tp_doc, "Ainakan Portal Membership" },
  { Py_tp_methods, PyPortalMembership_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.PortalService", PortalService, GObject, PyPortalService_init_from_handle, ainakan_unref,
  { Py_tp_doc, "Ainakan Portal Service" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.EndpointParameters", EndpointParameters, GObject, NULL, g_object_unref,
  { Py_tp_doc, "Ainakan EndpointParameters" },
  { Py_tp_init, PyEndpointParameters_init },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.Compiler", Compiler, GObject, NULL, ainakan_unref,
  { Py_tp_doc, "Ainakan Compiler" },
  { Py_tp_init, PyCompiler_init },
  { Py_tp_dealloc, PyCompiler_dealloc },
  { Py_tp_methods, PyCompiler_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.PackageManager", PackageManager, GObject, NULL, ainakan_unref,
  { Py_tp_doc, "Ainakan Package Manager" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.Package", Package, GObject, PyPackage_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Package" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.PackageSearchResult", PackageSearchResult, GObject, PyPackageSearchResult_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Package Search Result" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.PackageInstallResult", PackageInstallResult, GObject, PyPackageInstallResult_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan Package Install Result" },
...
PYAINAKAN_DEFINE_TYPE ("_ainakan.FileMonitor", FileMonitor, GObject, NULL, ainakan_unref,
  { Py_tp_doc, "Ainakan File Monitor" },
  { Py_tp_init, PyFileMonitor_init },
  { Py_tp_dealloc, PyFileMonitor_dealloc },
  { Py_tp_methods, PyFileMonitor_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.IOStream", IOStream, GObject, PyIOStream_init_from_handle, g_object_unref,
  { Py_tp_doc, "Ainakan IOStream" },
  { Py_tp_init, PyIOStream_init },
  { Py_tp_repr, PyIOStream_repr },
  { Py_tp_methods, PyIOStream_methods },
);

PYAINAKAN_DEFINE_TYPE ("_ainakan.Cancellable", Cancellable, GObject, NULL, g_object_unref,
  { Py_tp_doc, "Ainakan Cancellable" },
...
{
  self->handle = NULL;
  self->type = PYAINAKAN_TYPE (GObject);
...
PyGObject_on (PyGObject * self, PyObject * args)
{
  ...
  max_arg_count = PyAinakan_get_max_argument_count (callback);
...
{
  if (g_str_has_prefix (cname, "Ainakan"))
    return cname + 5;
...
PyGObject_marshal_socket_address (GSocketAddress * address)
{
  ...
  if (result == NULL)
    result = PyGObject_new_take_handle (g_object_ref (address), PYAINAKAN_TYPE (GObject));
...
propagate_error:
  {
    PyAinakan_raise (g_error_new_literal (AINAKAN_ERROR, AINAKAN_ERROR_INVALID_ARGUMENT, error->message));
    g_error_free (error);
...
PyDeviceManager_init (PyDeviceManager * self, PyObject * args, PyObject * kw)
{
  ...
  PyGObject_take_handle (&self->parent, ainakan_device_manager_new (), PYAINAKAN_TYPE (DeviceManager));
...
PyDeviceManager_dealloc (PyDeviceManager * self)
{
  ...
  {
    Py_BEGIN_ALLOW_THREADS
    ainakan_device_manager_close_sync (handle, NULL, NULL);
    ainakan_unref (handle);
    Py_END_ALLOW_THREADS
  }
...
PyDeviceManager_close (PyDeviceManager * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_manager_close_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDeviceManager_get_device_matching (PyDeviceManager * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_manager_get_device_sync (PY_GOBJECT_HANDLE (self), (AinakanDeviceManagerPredicate) PyDeviceManager_is_matching_device,
...
static gboolean
PyDeviceManager_is_matching_device (AinakanDevice * device, PyObject * predicate)
{
...
{
  GError * error = NULL;
  AinakanDeviceList * result;
  gint result_length, i;
  PyObject * devices;

  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_manager_enumerate_devices_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);

  result_length = ainakan_device_list_size (result);
  devices = PyList_New (result_length);
  for (i = 0; i != result_length; i++)
  {
    PyList_SetItem (devices, i, PyDevice_new_take_handle (ainakan_device_list_get (result, i)));
  }
  ainakan_unref (result);
...
PyDeviceManager_add_remote_device (PyDeviceManager * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanRemoteDeviceOptions * options;
  GError * error = NULL;
  AinakanDevice * handle;
  ...
  Py_BEGIN_ALLOW_THREADS
  handle = ainakan_device_manager_add_remote_device_sync (PY_GOBJECT_HANDLE (self), address, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS

  result = (error == NULL)
      ? PyDevice_new_take_handle (handle)
      : PyAinakan_raise (error);
...
PyDeviceManager_remove_remote_device (PyDeviceManager * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_manager_remove_remote_device_sync (PY_GOBJECT_HANDLE (self), address, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static AinakanRemoteDeviceOptions *
    ...
    gint keepalive_interval)
{
  ...
  options = ainakan_remote_device_options_new ();
  ...
  if (certificate_value != NULL)
  {
    ...
    ainakan_remote_device_options_set_certificate (options, certificate);

    g_object_unref (certificate);
  }

  if (origin != NULL)
    ainakan_remote_device_options_set_origin (options, origin);

  if (token != NULL)
    ainakan_remote_device_options_set_token (options, token);

  if (keepalive_interval != -1)
    ainakan_remote_device_options_set_keepalive_interval (options, keepalive_interval);
...
static PyObject *
PyDevice_new_take_handle (AinakanDevice * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Device));
}
...
static void
PyDevice_init_from_handle (PyDevice * self, AinakanDevice * handle)
{
  GVariant * icon;

  self->id = PyUnicode_FromString (ainakan_device_get_id (handle));
  self->name = PyUnicode_FromString (ainakan_device_get_name (handle));
  icon = ainakan_device_get_icon (handle);
  if (icon != NULL)
  {
  ...
  self->type = PyGObject_marshal_enum (ainakan_device_get_dtype (handle), AINAKAN_TYPE_DEVICE_TYPE);
  self->bus = PyBus_new_take_handle (g_object_ref (ainakan_device_get_bus (handle)));
}
...
PyDevice_is_lost (PyDevice * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  is_lost = ainakan_device_is_lost (PY_GOBJECT_HANDLE (self));
  Py_END_ALLOW_THREADS
...
PyDevice_query_system_parameters (PyDevice * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_query_system_parameters_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_get_frontmost_application (PyDevice * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanFrontmostQueryOptions * options;
  GError * error = NULL;
  AinakanApplication * result;

  if (!PyArg_ParseTupleAndKeywords (args, kw, "|s", keywords, &scope_value))
    return NULL;

  options = ainakan_frontmost_query_options_new ();

  if (scope_value != NULL)
  {
    AinakanScope scope;

    if (!PyGObject_unmarshal_enum (scope_value, AINAKAN_TYPE_SCOPE, &scope))
      goto invalid_argument;

    ainakan_frontmost_query_options_set_scope (options, scope);
  }

  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_get_frontmost_application_sync (PY_GOBJECT_HANDLE (self), options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PyDevice_enumerate_applications (PyDevice * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanApplicationQueryOptions * options;
  GError * error = NULL;
  AinakanApplicationList * result;
  gint result_length, i;
  ...
  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_enumerate_applications_sync (PY_GOBJECT_HANDLE (self), options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  ...
  result_length = ainakan_application_list_size (result);
  applications = PyList_New (result_length);
  for (i = 0; i != result_length; i++)
  {
    PyList_SetItem (applications, i, PyApplication_new_take_handle (ainakan_application_list_get (result, i)));
  }
...
PyDevice_parse_application_query_options (PyObject * identifiers_value, const gchar * scope_value)
{
  ...
  if (scope_value != NULL)
  {
    ...
    ainakan_application_query_options_set_scope (options, scope);
  }
...
PyDevice_enumerate_processes (PyDevice * self, PyObject * args, PyObject * kw)
{
  ...
  PyObject * pids = NULL;
  const char * scope = NULL;
  AinakanProcessQueryOptions * options;
  GError * error = NULL;
  AinakanProcessList * result;
  gint result_length, i;
  PyObject * processes;
  ...
  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_enumerate_processes_sync (PY_GOBJECT_HANDLE (self), options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  ...
  result_length = ainakan_process_list_size (result);
  processes = PyList_New (result_length);
  for (i = 0; i != result_length; i++)
  {
    PyList_SetItem (processes, i, PyProcess_new_take_handle (ainakan_process_list_get (result, i)));
  }
  g_object_unref (result);
...
PyDevice_parse_process_query_options (PyObject * pids_value, const gchar * scope_value)
{
  ...
  if (pids_value != NULL)
  {
    ...
    for (i = 0; i != n; i++)
    {
      ...
      ainakan_process_query_options_select_pid (options, pid);
    }
  }
  ...
  if (scope_value != NULL)
  {
    ...
    ainakan_process_query_options_set_scope (options, scope);
  }
...
PyDevice_enable_spawn_gating (PyDevice * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_enable_spawn_gating_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_disable_spawn_gating (PyDevice * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_disable_spawn_gating_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
{
  GError * error = NULL;
  AinakanSpawnList * result;
  gint result_length, i;
  PyObject * spawn;

  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_enumerate_pending_spawn_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);

  result_length = ainakan_spawn_list_size (result);
  spawn = PyList_New (result_length);
  for (i = 0; i != result_length; i++)
  {
    PyList_SetItem (spawn, i, PySpawn_new_take_handle (ainakan_spawn_list_get (result, i)));
  }
  g_object_unref (result);
...
{
  GError * error = NULL;
  AinakanChildList * result;
  gint result_length, i;
  PyObject * children;

  Py_BEGIN_ALLOW_THREADS
  result = ainakan_device_enumerate_pending_children_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);

  result_length = ainakan_child_list_size (result);
  children = PyList_New (result_length);
  for (i = 0; i != result_length; i++)
  {
    PyList_SetItem (children, i, PyChild_new_take_handle (ainakan_child_list_get (result, i)));
  }
  g_object_unref (result);
...
PyDevice_spawn (PyDevice * self, PyObject * args, PyObject * kw)
{
  ...
  options = ainakan_spawn_options_new ();
  ...
  if (argv_value != Py_None)
  {
    ...
    ainakan_spawn_options_set_argv (options, argv, argv_length);
  ...
  if (envp_value != Py_None)
  {
    ...
    ainakan_spawn_options_set_envp (options, envp, envp_length);
  ...
  if (env_value != Py_None)
  {
    ...
    ainakan_spawn_options_set_env (options, env, env_length);
  ...
  if (stdio_value != NULL)
  {
    ...
    if (!PyGObject_unmarshal_enum (stdio_value, AINAKAN_TYPE_STDIO, &stdio))
      goto invalid_argument;

    ainakan_spawn_options_set_stdio (options, stdio);
  }
  ...
  if (aux_value != Py_None)
  {
    ...
    aux = ainakan_spawn_options_get_aux (options);
  ...
  Py_BEGIN_ALLOW_THREADS
  pid = ainakan_device_spawn_sync (PY_GOBJECT_HANDLE (self), program, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PyDevice_input (PyDevice * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_input_sync (PY_GOBJECT_HANDLE (self), (guint) pid, data, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PyDevice_resume (PyDevice * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_resume_sync (PY_GOBJECT_HANDLE (self), (guint) pid, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_kill (PyDevice * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_kill_sync (PY_GOBJECT_HANDLE (self), (guint) pid, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_attach (PyDevice * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanSessionOptions * options = NULL;
  GError * error = NULL;
  AinakanSession * handle;
  ...
  Py_BEGIN_ALLOW_THREADS
  handle = ainakan_device_attach_sync (PY_GOBJECT_HANDLE (self), (guint) pid, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static AinakanSessionOptions *
PyDevice_parse_session_options (const gchar * realm_value,
                                guint persist_timeout)
{
  AinakanSessionOptions * options;

  options = ainakan_session_options_new ();

  if (realm_value != NULL)
  {
    AinakanRealm realm;

    if (!PyGObject_unmarshal_enum (realm_value, AINAKAN_TYPE_REALM, &realm))
      goto propagate_error;

    ainakan_session_options_set_realm (options, realm);
  }

  ainakan_session_options_set_persist_timeout (options, persist_timeout);
...
PyDevice_inject_library_file (PyDevice * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  id = ainakan_device_inject_library_file_sync (PY_GOBJECT_HANDLE (self), (guint) pid, path, entrypoint, data, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_inject_library_blob (PyDevice * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  id = ainakan_device_inject_library_blob_sync (PY_GOBJECT_HANDLE (self), (guint) pid, blob, entrypoint, data, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PyDevice_open_channel (PyDevice * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  stream = ainakan_device_open_channel_sync (PY_GOBJECT_HANDLE (self), address, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_open_service (PyDevice * self, PyObject * args)
{
  const char * address;
  GError * error = NULL;
  AinakanService * service;

  if (!PyArg_ParseTuple (args, "s", &address))
    return NULL;

  Py_BEGIN_ALLOW_THREADS
  service = ainakan_device_open_service_sync (PY_GOBJECT_HANDLE (self), address, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyDevice_unpair (PyDevice * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_device_unpair_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
static PyObject *
PyApplication_new_take_handle (AinakanApplication * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Application));
}
...
static void
PyApplication_init_from_handle (PyApplication * self, AinakanApplication * handle)
{
  self->identifier = PyUnicode_FromString (ainakan_application_get_identifier (handle));
  self->name = PyUnicode_FromString (ainakan_application_get_name (handle));
  self->pid = ainakan_application_get_pid (handle);
  self->parameters = PyApplication_marshal_parameters_dict (ainakan_application_get_parameters (handle));
}
...
PyApplication_repr (PyApplication * self)
{
  ...
  g_string_append_printf (repr, "identifier=\"%s\", name=\"%s\"",
      ainakan_application_get_identifier (handle),
      ainakan_application_get_name (handle));

  if (self->pid != 0)
    g_string_append_printf (repr, ", pid=%u", self->pid);

  str = PyAinakan_repr (self->parameters);
...
static PyObject *
PyProcess_new_take_handle (AinakanProcess * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Process));
}
...
static void
PyProcess_init_from_handle (PyProcess * self, AinakanProcess * handle)
{
  self->pid = ainakan_process_get_pid (handle);
  self->name = PyUnicode_FromString (ainakan_process_get_name (handle));
  self->parameters = PyProcess_marshal_parameters_dict (ainakan_process_get_parameters (handle));
}
...
PyProcess_repr (PyProcess * self)
{
  ...
  g_string_append_printf (repr, "pid=%u, name=\"%s\"",
      self->pid,
      ainakan_process_get_name (handle));
...
static PyObject *
PySpawn_new_take_handle (AinakanSpawn * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Spawn));
}
...
static void
PySpawn_init_from_handle (PySpawn * self, AinakanSpawn * handle)
{
  self->pid = ainakan_spawn_get_pid (handle);
  self->identifier = PyGObject_marshal_string (ainakan_spawn_get_identifier (handle));
}
...
static PyObject *
PyChild_new_take_handle (AinakanChild * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Child));
}
...
static void
PyChild_init_from_handle (PyChild * self, AinakanChild * handle)
{
  gchar * const * argv, * const * envp;
  gint argv_length, envp_length;

  self->pid = ainakan_child_get_pid (handle);
  self->parent_pid = ainakan_child_get_parent_pid (handle);

  self->origin = PyGObject_marshal_enum (ainakan_child_get_origin (handle), AINAKAN_TYPE_CHILD_ORIGIN);

  self->identifier = PyGObject_marshal_string (ainakan_child_get_identifier (handle));

  self->path = PyGObject_marshal_string (ainakan_child_get_path (handle));

  argv = ainakan_child_get_argv (handle, &argv_length);
  self->argv = PyGObject_marshal_strv (argv, argv_length);

  envp = ainakan_child_get_envp (handle, &envp_length);
...
PyChild_repr (PyChild * self)
{
  ...
  origin = ainakan_child_get_origin (handle);
  origin_class = g_type_class_ref (AINAKAN_TYPE_CHILD_ORIGIN);
  ...
  if (self->identifier != Py_None)
  {
    ...
    identifier = PyAinakan_repr (self->identifier);
  ...
  if (origin != AINAKAN_CHILD_ORIGIN_FORK)
  {
    gchar * path, * argv, * envp;

    path = PyAinakan_repr (self->path);
    argv = PyAinakan_repr (self->argv);
    envp = PyAinakan_repr (self->envp);
...
static void
PyCrash_init_from_handle (PyCrash * self, AinakanCrash * handle)
{
  self->pid = ainakan_crash_get_pid (handle);
  self->process_name = PyGObject_marshal_string (ainakan_crash_get_process_name (handle));
  self->summary = PyGObject_marshal_string (ainakan_crash_get_summary (handle));
  self->report = PyGObject_marshal_string (ainakan_crash_get_report (handle));
  self->parameters = PyGObject_marshal_parameters_dict (ainakan_crash_get_parameters (handle));
}
...
PyCrash_repr (PyCrash * self)
{
  ...
  g_string_append_printf (repr, "pid=%u, process_name=\"%s\", summary=\"%s\", report=<%u bytes>",
      self->pid,
      ainakan_crash_get_process_name (handle),
      ainakan_crash_get_summary (handle),
      (guint) strlen (ainakan_crash_get_report (handle)));
...
static PyObject *
PyBus_new_take_handle (AinakanBus * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Bus));
}
...
PyBus_attach (PySession * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_bus_attach_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyBus_post (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_bus_post (PY_GOBJECT_HANDLE (self), message, data);
  Py_END_ALLOW_THREADS
...
static PyObject *
PyService_new_take_handle (AinakanService * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Service));
}
...
PyService_activate (PyService * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_service_activate_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyService_cancel (PyService * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_service_cancel_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyService_request (PyService * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  raw_result = ainakan_service_request_sync (PY_GOBJECT_HANDLE (self), raw_params, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static PyObject *
PySession_new_take_handle (AinakanSession * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Session));
}
...
static void
PySession_init_from_handle (PySession * self, AinakanSession * handle)
{
  self->pid = ainakan_session_get_pid (handle);
}
...
PySession_is_detached (PySession * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  is_detached = ainakan_session_is_detached (PY_GOBJECT_HANDLE (self));
  Py_END_ALLOW_THREADS
...
PySession_detach (PySession * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_session_detach_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PySession_resume (PySession * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_session_resume_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PySession_enable_child_gating (PySession * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_session_enable_child_gating_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PySession_disable_child_gating (PySession * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_session_disable_child_gating_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PySession_create_script (PySession * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanScriptOptions * options;
  GError * error = NULL;
  AinakanScript * handle;
  ...
  Py_BEGIN_ALLOW_THREADS
  handle = ainakan_session_create_script_sync (PY_GOBJECT_HANDLE (self), source, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PySession_create_script_from_bytes (PySession * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  handle = ainakan_session_create_script_from_bytes_sync (PY_GOBJECT_HANDLE (self), bytes, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PySession_compile_script (PySession * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  bytes = ainakan_session_compile_script_sync (PY_GOBJECT_HANDLE (self), source, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PySession_parse_script_options (const gchar * name, gconstpointer snapshot_data, gsize snapshot_size, const gchar * runtime_value)
{
  ...
  options = ainakan_script_options_new ();

  if (name != NULL)
    ainakan_script_options_set_name (options, name);

  if (snapshot_data != NULL)
  {
    GBytes * snapshot = g_bytes_new (snapshot_data, snapshot_size);
    ainakan_script_options_set_snapshot (options, snapshot);
    g_bytes_unref (snapshot);
  }

  if (runtime_value != NULL)
  {
    AinakanScriptRuntime runtime;

    if (!PyGObject_unmarshal_enum (runtime_value, AINAKAN_TYPE_SCRIPT_RUNTIME, &runtime))
      goto invalid_argument;

    ainakan_script_options_set_runtime (options, runtime);
  }
...
PySession_snapshot_script (PySession * self, PyObject * args, PyObject * kw)
{
  ...
  const char * runtime_value = NULL;
  AinakanSnapshotOptions * options;
  GError * error = NULL;
  GBytes * bytes;
  ...
  Py_BEGIN_ALLOW_THREADS
  bytes = ainakan_session_snapshot_script_sync (PY_GOBJECT_HANDLE (self), embed_script, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static AinakanSnapshotOptions *
PySession_parse_snapshot_options (const gchar * warmup_script, const gchar * runtime_value)
{
  AinakanSnapshotOptions * options;

  options = ainakan_snapshot_options_new ();

  if (warmup_script != NULL)
    ainakan_snapshot_options_set_warmup_script (options, warmup_script);
  ...
  if (runtime_value != NULL)
  {
    ...
    ainakan_snapshot_options_set_runtime (options, runtime);
  }
...
PySession_setup_peer_connection (PySession * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanPeerOptions * options = NULL;
  GError * error = NULL;
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_session_setup_peer_connection_sync (PY_GOBJECT_HANDLE (self), options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static AinakanPeerOptions *
PySession_parse_peer_options (const gchar * stun_server, PyObject * relays)
{
  AinakanPeerOptions * options;
  PyObject * relay;

  options = ainakan_peer_options_new ();

  ainakan_peer_options_set_stun_server (options, stun_server);
  ...
  if (relays != NULL)
  {
    ...
    for (i = 0; i != n; i++)
    {
      ...
      if (!PyObject_IsInstance (relay, PYAINAKAN_TYPE_OBJECT (Relay)))
        goto expected_relay;

      ainakan_peer_options_add_relay (options, PY_GOBJECT_HANDLE (relay));
...
PySession_join_portal (PySession * self, PyObject * args, PyObject * kw)
{
  ...
  char * token = NULL;
  PyObject * acl = NULL;
  AinakanPortalOptions * options;
  GError * error = NULL;
  AinakanPortalMembership * handle;
  ...
  Py_BEGIN_ALLOW_THREADS
  handle = ainakan_session_join_portal_sync (PY_GOBJECT_HANDLE (self), address, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PySession_parse_portal_options (const gchar * certificate_value, const gchar * token, PyObject * acl_value)
{
  ...
  if (certificate_value != NULL)
  {
    ...
    ainakan_portal_options_set_certificate (options, certificate);

    g_object_unref (certificate);
  }

  if (token != NULL)
    ainakan_portal_options_set_token (options, token);
  ...
  if (acl_value != NULL)
  {
    ...
    ainakan_portal_options_set_acl (options, acl, acl_length);
...
static PyObject *
PyScript_new_take_handle (AinakanScript * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Script));
}
...
PyScript_is_destroyed (PyScript * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  is_destroyed = ainakan_script_is_destroyed (PY_GOBJECT_HANDLE (self));
  Py_END_ALLOW_THREADS
...
PyScript_load (PyScript * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_script_load_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyScript_unload (PyScript * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_script_unload_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyScript_eternalize (PyScript * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_script_eternalize_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyScript_post (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_script_post (PY_GOBJECT_HANDLE (self), message, data);
  Py_END_ALLOW_THREADS
...
PyScript_enable_debugger (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_script_enable_debugger_sync (PY_GOBJECT_HANDLE (self), port, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyScript_disable_debugger (PyScript * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_script_disable_debugger_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyRelay_init (PyRelay * self, PyObject * args, PyObject * kw)
{
  ...
  if (!PyGObject_unmarshal_enum (kind_value, AINAKAN_TYPE_RELAY_KIND, &kind))
    goto beach;

  handle = ainakan_relay_new (address, username, password, kind);

  PyGObject_take_handle (&self->parent, handle, PYAINAKAN_TYPE (Relay));
...
static void
PyRelay_init_from_handle (PyRelay * self, AinakanRelay * handle)
{
  self->address = PyUnicode_FromString (ainakan_relay_get_address (handle));
  self->username = PyUnicode_FromString (ainakan_relay_get_username (handle));
  self->password = PyUnicode_FromString (ainakan_relay_get_password (handle));
  self->kind = PyGObject_marshal_enum (ainakan_relay_get_kind (handle), AINAKAN_TYPE_RELAY_KIND);
}
...
static PyObject *
PyPortalMembership_new_take_handle (AinakanPortalMembership * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (PortalMembership));
}
...
PyPortalMembership_terminate (PyPortalMembership * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_membership_terminate_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyPortalService_init (PyPortalService * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanPortalService * handle;

  if (PyGObject_tp_init ((PyObject *) self, args, kw) < 0)
    return -1;

  if (!PyArg_ParseTupleAndKeywords (args, kw, "O!|O!", keywords,
        PYAINAKAN_TYPE_OBJECT (EndpointParameters), &cluster_params,
        PYAINAKAN_TYPE_OBJECT (EndpointParameters), &control_params))
    return -1;

  g_atomic_int_inc (&toplevel_objects_alive);

  handle = ainakan_portal_service_new (PY_GOBJECT_HANDLE (cluster_params),
      (control_params != NULL) ? PY_GOBJECT_HANDLE (control_params) : NULL);

  PyGObject_take_handle (&self->parent, handle, PYAINAKAN_TYPE (PortalService));
...
static void
PyPortalService_init_from_handle (PyPortalService * self, AinakanPortalService * handle)
{
  self->device = PyDevice_new_take_handle (g_object_ref (ainakan_portal_service_get_device (handle)));
}
...
PyPortalService_dealloc (PyPortalService * self)
{
  ...
  {
    Py_BEGIN_ALLOW_THREADS
    ainakan_portal_service_stop_sync (handle, NULL, NULL);
    ainakan_unref (handle);
    Py_END_ALLOW_THREADS
  }
...
PyPortalService_start (PyPortalService * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_start_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyPortalService_stop (PyPortalService * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_stop_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyPortalService_kick (PyScript * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_kick (PY_GOBJECT_HANDLE (self), connection_id);
  Py_END_ALLOW_THREADS
...
PyPortalService_post (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_post (PY_GOBJECT_HANDLE (self), connection_id, message, data);
  Py_END_ALLOW_THREADS
...
PyPortalService_narrowcast (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_narrowcast (PY_GOBJECT_HANDLE (self), tag, message, data);
  Py_END_ALLOW_THREADS
...
PyPortalService_broadcast (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_broadcast (PY_GOBJECT_HANDLE (self), message, data);
  Py_END_ALLOW_THREADS
...
PyPortalService_enumerate_tags (PyScript * self, PyObject * args)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  tags = ainakan_portal_service_enumerate_tags (PY_GOBJECT_HANDLE (self), connection_id, &tags_length);
  Py_END_ALLOW_THREADS
...
PyPortalService_tag (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_tag (PY_GOBJECT_HANDLE (self), connection_id, tag);
  Py_END_ALLOW_THREADS
...
PyPortalService_untag (PyScript * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_portal_service_untag (PY_GOBJECT_HANDLE (self), connection_id, tag);
  Py_END_ALLOW_THREADS
...
PyEndpointParameters_init (PyEndpointParameters * self, PyObject * args, PyObject * kw)
{
  ...
  AinakanAuthenticationService * auth_service = NULL;
  GFile * asset_root = NULL;
  AinakanEndpointParameters * handle;
  ...
  if (auth_token != NULL)
    auth_service = AINAKAN_AUTHENTICATION_SERVICE (ainakan_static_authentication_service_new (auth_token));
  else if (auth_callback != NULL)
    auth_service = AINAKAN_AUTHENTICATION_SERVICE (ainakan_python_authentication_service_new (auth_callback));

  if (asset_root_value != NULL)
    asset_root = g_file_new_for_path (asset_root_value);

  handle = ainakan_endpoint_parameters_new (address, port, certificate, origin, auth_service, asset_root);

  PyGObject_take_handle (&self->parent, handle, PYAINAKAN_TYPE (EndpointParameters));
...
G_DEFINE_TYPE_EXTENDED (AinakanPythonAuthenticationService, ainakan_python_authentication_service, G_TYPE_OBJECT, 0,
    G_IMPLEMENT_INTERFACE (AINAKAN_TYPE_AUTHENTICATION_SERVICE, ainakan_python_authentication_service_iface_init))

static AinakanPythonAuthenticationService *
ainakan_python_authentication_service_new (PyObject * callback)
{
  AinakanPythonAuthenticationService * service;

  service = g_object_new (AINAKAN_TYPE_PYTHON_AUTHENTICATION_SERVICE, NULL);
...
static void
ainakan_python_authentication_service_class_init (AinakanPythonAuthenticationServiceClass * klass)
{
  GObjectClass * object_class = G_OBJECT_CLASS (klass);

  object_class->dispose = ainakan_python_authentication_service_dispose;
}

static void
ainakan_python_authentication_service_iface_init (gpointer g_iface, gpointer iface_data)
{
  AinakanAuthenticationServiceIface * iface = g_iface;

  iface->authenticate = ainakan_python_authentication_service_authenticate;
  iface->authenticate_finish = ainakan_python_authentication_service_authenticate_finish;
}

static void
ainakan_python_authentication_service_init (AinakanPythonAuthenticationService * self)
{
  self->pool = g_thread_pool_new ((GFunc) ainakan_python_authentication_service_do_authenticate, self, 1, FALSE, NULL);
}

static void
ainakan_python_authentication_service_dispose (GObject * object)
{
  AinakanPythonAuthenticationService * self = AINAKAN_PYTHON_AUTHENTICATION_SERVICE (object);
  ...
  G_OBJECT_CLASS (ainakan_python_authentication_service_parent_class)->dispose (object);
}

static void
ainakan_python_authentication_service_authenticate (AinakanAuthenticationService * service, const gchar * token, GCancellable * cancellable,
    GAsyncReadyCallback callback, gpointer user_data)
{
  AinakanPythonAuthenticationService * self;
  GTask * task;

  self = AINAKAN_PYTHON_AUTHENTICATION_SERVICE (service);
...
static gchar *
ainakan_python_authentication_service_authenticate_finish (AinakanAuthenticationService * service, GAsyncResult * result, GError ** error)
{
  return g_task_propagate_pointer (G_TASK (result), error);
}

static void
ainakan_python_authentication_service_do_authenticate (GTask * task, AinakanPythonAuthenticationService * self)
{
  const gchar * token;
  PyGILState_STATE gstate;
  PyObject * result;
  ...
  if (session_info != NULL)
    ...
    g_task_return_new_error (task, AINAKAN_ERROR, AINAKAN_ERROR_INVALID_ARGUMENT, "%s", message);
...
PyCompiler_init (PyCompiler * self, PyObject * args, PyObject * kw)
{
  ...
  PyGObject_take_handle (&self->parent, ainakan_compiler_new (NULL), PYAINAKAN_TYPE (Compiler));
...
PyCompiler_build (PyCompiler * self, PyObject * args, PyObject * kw)
{
  ...
  options = ainakan_build_options_new ();
  if (!PyCompiler_set_options (AINAKAN_COMPILER_OPTIONS (options), project_root, output_format, bundle_format, type_check, source_maps,
        compression, platform, externals))
    goto invalid_option_value;

  Py_BEGIN_ALLOW_THREADS
  bundle = ainakan_compiler_build_sync (PY_GOBJECT_HANDLE (self), entrypoint, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PyCompiler_watch (PyCompiler * self, PyObject * args, PyObject * kw)
{
  ...
  options = ainakan_watch_options_new ();
  if (!PyCompiler_set_options (AINAKAN_COMPILER_OPTIONS (options), project_root, output_format, bundle_format, type_check, source_maps,
        compression, platform, externals))
    goto invalid_option_value;

  Py_BEGIN_ALLOW_THREADS
  ainakan_compiler_watch_sync (PY_GOBJECT_HANDLE (self), entrypoint, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static gboolean
PyCompiler_set_options (AinakanCompilerOptions * options, const gchar * project_root_value, const gchar * output_format_value,
    const gchar * bundle_format_value, const gchar * type_check_value, const gchar * source_maps_value, const gchar * compression_value,
    const gchar * platform_value, PyObject * externals_value)
{
  if (project_root_value != NULL)
    ainakan_compiler_options_set_project_root (options, project_root_value);

  if (output_format_value != NULL)
  {
    AinakanOutputFormat output_format;

    if (!PyGObject_unmarshal_enum (output_format_value, AINAKAN_TYPE_OUTPUT_FORMAT, &output_format))
      return FALSE;

    ainakan_compiler_options_set_output_format (options, output_format);
  }

  if (bundle_format_value != NULL)
  {
    AinakanBundleFormat bundle_format;

    if (!PyGObject_unmarshal_enum (bundle_format_value, AINAKAN_TYPE_BUNDLE_FORMAT, &bundle_format))
      return FALSE;

    ainakan_compiler_options_set_bundle_format (options, bundle_format);
  }

  if (type_check_value != NULL)
  {
    AinakanTypeCheckMode type_check;

    if (!PyGObject_unmarshal_enum (type_check_value, AINAKAN_TYPE_TYPE_CHECK_MODE, &type_check))
      return FALSE;

    ainakan_compiler_options_set_type_check (options, type_check);
  }

  if (source_maps_value != NULL)
  {
    AinakanSourceMaps source_maps;

    if (!PyGObject_unmarshal_enum (source_maps_value, AINAKAN_TYPE_SOURCE_MAPS, &source_maps))
      return FALSE;

    ainakan_compiler_options_set_source_maps (options, source_maps);
  }

  if (compression_value != NULL)
  {
    AinakanJsCompression compression;

    if (!PyGObject_unmarshal_enum (compression_value, AINAKAN_TYPE_JS_COMPRESSION, &compression))
      return FALSE;

    ainakan_compiler_options_set_compression (options, compression);
  }

  if (platform_value != NULL)
  {
    AinakanJsPlatform platform;

    if (!PyGObject_unmarshal_enum (platform_value, AINAKAN_TYPE_JS_PLATFORM, &platform))
      return FALSE;

    ainakan_compiler_options_set_platform (options, platform);
  }
  ...
  if (externals_value != NULL)
  {
    ...
    for (i = 0; i != n; i++)
    {
      ...
      ainakan_compiler_options_add_external (options, external);
...
PyPackageManager_init (PyPackageManager * self, PyObject * args, PyObject * kw)
{
  ...
  PyGObject_take_handle (&self->parent, ainakan_package_manager_new (), PYAINAKAN_TYPE (PackageManager));
...
PyPackageManager_repr (PyPackageManager * self)
{
  ...
  repr = g_strdup_printf ("PackageManager(registry=\"%s\")", ainakan_package_manager_get_registry (PY_GOBJECT_HANDLE (self)));
...
{
  return PyUnicode_FromString (ainakan_package_manager_get_registry (PY_GOBJECT_HANDLE (self)));
}
...
PyPackageManager_set_registry (PyPackageManager * self, PyObject * val, void * closure)
{
  ...
  if (!PyGObject_unmarshal_string (val, &registry))
    return -1;
  ainakan_package_manager_set_registry (PY_GOBJECT_HANDLE (self), registry);
  g_free (registry);
...
{
  AinakanPackageSearchResult * result;
  static char * keywords[] = { "query", "offset", "limit", NULL };
  const char * query;
  guint offset = G_MAXUINT;
  guint limit = G_MAXUINT;
  AinakanPackageSearchOptions * options;
  GError * error = NULL;

  if (!PyArg_ParseTupleAndKeywords (args, kw, "s|II", keywords, &query, &offset, &limit))
    return NULL;

  options = ainakan_package_search_options_new ();

  if (offset != G_MAXUINT)
    ainakan_package_search_options_set_offset (options, offset);

  if (limit != G_MAXUINT)
    ainakan_package_search_options_set_limit (options, limit);

  Py_BEGIN_ALLOW_THREADS
  result = ainakan_package_manager_search_sync (PY_GOBJECT_HANDLE (self), query, options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
PyPackageManager_install (PyPackageManager * self, PyObject * args, PyObject * kw)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  result = ainakan_package_manager_install_sync (PY_GOBJECT_HANDLE (self), options, g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
...
static AinakanPackageInstallOptions *
PyPackageManager_parse_install_options (const gchar * project_root, const char * role_value, PyObject * specs_value, PyObject * omits_value)
{
  ...
  options = ainakan_package_install_options_new ();

  if (project_root != NULL)
    ainakan_package_install_options_set_project_root (options, project_root);

  if (role_value != NULL)
  {
    AinakanPackageRole role;

    if (!PyGObject_unmarshal_enum (role_value, AINAKAN_TYPE_PACKAGE_ROLE, &role))
      goto propagate_error;

    ainakan_package_install_options_set_role (options, role);
  }
  ...
  if (specs_value != NULL)
  {
    ...
    for (i = 0; i != n; i++)
    {
      ...
      ainakan_package_install_options_add_spec (options, spec);
  ...
  if (omits_value != NULL)
  {
    ...
    for (i = 0; i != n; i++)
    {
      ...
      if (!PyGObject_unmarshal_enum (str, AINAKAN_TYPE_PACKAGE_ROLE, &role))
        goto propagate_error;

      ainakan_package_install_options_add_omit (options, role);
    }
  }
...
static PyObject *
PyPackage_new_take_handle (AinakanPackage * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (Package));
}
...
static void
PyPackage_init_from_handle (PyPackage * self, AinakanPackage * handle)
{
  self->name = PyUnicode_FromString (ainakan_package_get_name (handle));
  self->version = PyUnicode_FromString (ainakan_package_get_version (handle));
  self->description = PyGObject_marshal_string (ainakan_package_get_description (handle));
  self->url = PyGObject_marshal_string (ainakan_package_get_url (handle));
}
...
PyPackage_repr (PyPackage * self)
{
  ...
  g_string_append_printf (repr, "Package(name=\"%s\", version=\"%s\"",
      ainakan_package_get_name (handle),
      ainakan_package_get_version (handle));

  description = ainakan_package_get_description (handle);
  if (description != NULL)
  {
  ...
  url = ainakan_package_get_url (handle);
  if (url != NULL)
...
static PyObject *
PyPackageList_marshal (AinakanPackageList * list)
{
  PyObject * result;
  gint n, i;

  n = ainakan_package_list_size (list);
  result = PyList_New (n);
  for (i = 0; i != n; i++)
    PyList_SetItem (result, i, PyPackage_new_take_handle (ainakan_package_list_get (list, i)));
...
static PyObject *
PyPackageSearchResult_new_take_handle (AinakanPackageSearchResult * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (PackageSearchResult));
}
...
static void
PyPackageSearchResult_init_from_handle (PyPackageSearchResult * self, AinakanPackageSearchResult * handle)
{
  self->packages = PyPackageList_marshal (ainakan_package_search_result_get_packages (handle));
  self->total = ainakan_package_search_result_get_total (handle);
}
...
PyPackageSearchResult_repr (PyPackageSearchResult * self)
{
  ...
  num_packages = ainakan_package_list_size (ainakan_package_search_result_get_packages (PY_GOBJECT_HANDLE (self)));
  if (num_packages != 0)
...
static PyObject *
PyPackageInstallResult_new_take_handle (AinakanPackageInstallResult * handle)
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (PackageInstallResult));
}
...
static void
PyPackageInstallResult_init_from_handle (PyPackageInstallResult * self, AinakanPackageInstallResult * handle)
{
  self->packages = PyPackageList_marshal (ainakan_package_install_result_get_packages (handle));
}
...
PyPackageInstallResult_repr (PyPackageInstallResult * self)
{
  ...
  num_packages = ainakan_package_list_size (ainakan_package_install_result_get_packages (PY_GOBJECT_HANDLE (self)));
  if (num_packages != 0)
    g_string_append_printf (repr, "[<%u package%s>]", num_packages, (num_packages == 1) ? "" : "s");
  else
    g_string_append (repr, "[]");
...
PyFileMonitor_init (PyFileMonitor * self, PyObject * args, PyObject * kw)
{
  ...
  PyGObject_take_handle (&self->parent, ainakan_file_monitor_new (path), PYAINAKAN_TYPE (FileMonitor));
...
PyFileMonitor_enable (PyFileMonitor * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_file_monitor_enable_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
PyFileMonitor_disable (PyFileMonitor * self)
{
  ...
  Py_BEGIN_ALLOW_THREADS
  ainakan_file_monitor_disable_sync (PY_GOBJECT_HANDLE (self), g_cancellable_get_current (), &error);
  Py_END_ALLOW_THREADS
  if (error != NULL)
    return PyAinakan_raise (error);
...
{
  return PyGObject_new_take_handle (handle, PYAINAKAN_TYPE (IOStream));
}
...
PyCancellable_new_take_handle (GCancellable * handle)
{
  ...
  if (object == NULL)
  {
    object = PyObject_CallFunction (PYAINAKAN_TYPE_OBJECT (Cancellable), "z#", (char *) &handle, (Py_ssize_t) sizeof (handle));
  }
  else
  {
...
PyCancellable_init (PyCancellable * self, PyObject * args, PyObject * kw)
{
  ...
  PyGObject_take_handle (&self->parent, handle, PYAINAKAN_TYPE (Cancellable));
...
invalid_operation:
  {
    return PyAinakan_raise (g_error_new (
          AINAKAN_ERROR,
          AINAKAN_ERROR_INVALID_OPERATION,
...
static void
PyAinakan_object_decref (gpointer obj)
{
  PyObject * o = obj;
  Py_DecRef (o);
}
...
static PyObject *
PyAinakan_raise (GError * error)
{
  PyObject * exception;
  GString * message;

  if (error->domain == AINAKAN_ERROR)
  {
    exception = g_hash_table_lookup (ainakan_exception_by_error_code, GINT_TO_POINTER (error->code));
...
static guint
PyAinakan_get_max_argument_count (PyObject * callable)
{
...
PyInit__ainakan (void)
{
  ...
  module = PyModule_Create (&PyAinakan_moduledef);

  PyModule_AddStringConstant (module, "__version__", ainakan_version_string ());

  PYAINAKAN_REGISTER_TYPE (GObject, G_TYPE_OBJECT);
  PyGObject_tp_init = PyType_GetSlot ((PyTypeObject *) PYAINAKAN_TYPE_OBJECT (GObject), Py_tp_init);
  PyGObject_tp_dealloc = PyType_GetSlot ((PyTypeObject *) PYAINAKAN_TYPE_OBJECT (GObject), Py_tp_dealloc);

  PYAINAKAN_REGISTER_TYPE (DeviceManager, AINAKAN_TYPE_DEVICE_MANAGER);
  PYAINAKAN_REGISTER_TYPE (Device, AINAKAN_TYPE_DEVICE);
  PYAINAKAN_REGISTER_TYPE (Application, AINAKAN_TYPE_APPLICATION);
  PYAINAKAN_REGISTER_TYPE (Process, AINAKAN_TYPE_PROCESS);
  PYAINAKAN_REGISTER_TYPE (Spawn, AINAKAN_TYPE_SPAWN);
  PYAINAKAN_REGISTER_TYPE (Child, AINAKAN_TYPE_CHILD);
  PYAINAKAN_REGISTER_TYPE (Crash, AINAKAN_TYPE_CRASH);
  PYAINAKAN_REGISTER_TYPE (Bus, AINAKAN_TYPE_BUS);
  PYAINAKAN_REGISTER_TYPE (Service, AINAKAN_TYPE_SERVICE);
  PYAINAKAN_REGISTER_TYPE (Session, AINAKAN_TYPE_SESSION);
  PYAINAKAN_REGISTER_TYPE (Script, AINAKAN_TYPE_SCRIPT);
  PYAINAKAN_REGISTER_TYPE (Relay, AINAKAN_TYPE_RELAY);
  PYAINAKAN_REGISTER_TYPE (PortalMembership, AINAKAN_TYPE_PORTAL_MEMBERSHIP);
  PYAINAKAN_REGISTER_TYPE (PortalService, AINAKAN_TYPE_PORTAL_SERVICE);
  PYAINAKAN_REGISTER_TYPE (EndpointParameters, AINAKAN_TYPE_ENDPOINT_PARAMETERS);
  PYAINAKAN_REGISTER_TYPE (Compiler, AINAKAN_TYPE_COMPILER);
  PYAINAKAN_REGISTER_TYPE (PackageManager, AINAKAN_TYPE_PACKAGE_MANAGER);
  PYAINAKAN_REGISTER_TYPE (Package, AINAKAN_TYPE_PACKAGE);
  PYAINAKAN_REGISTER_TYPE (PackageSearchResult, AINAKAN_TYPE_PACKAGE_SEARCH_RESULT);
  PYAINAKAN_REGISTER_TYPE (PackageInstallResult, AINAKAN_TYPE_PACKAGE_INSTALL_RESULT);
  PYAINAKAN_REGISTER_TYPE (FileMonitor, AINAKAN_TYPE_FILE_MONITOR);
  PYAINAKAN_REGISTER_TYPE (IOStream, G_TYPE_IO_STREAM);
  PYAINAKAN_REGISTER_TYPE (Cancellable, G_TYPE_CANCELLABLE);

  ainakan_exception_by_error_code = g_hash_table_new_full (NULL, NULL, NULL, PyAinakan_object_decref);
#define PYAINAKAN_DECLARE_EXCEPTION(code, name) \
    do \
    { \
      PyObject * exception = PyErr_NewException ("ainakan." name "Error", NULL, NULL); \
      g_hash_table_insert (ainakan_exception_by_error_code, GINT_TO_POINTER (G_PASTE (AINAKAN_ERROR_, code)), exception); \
      Py_IncRef (exception); \
      PyModule_AddObject (module, name "Error", exception); \
    } while (FALSE)
  PYAINAKAN_DECLARE_EXCEPTION (SERVER_NOT_RUNNING, "ServerNotRunning");
  PYAINAKAN_DECLARE_EXCEPTION (EXECUTABLE_NOT_FOUND, "ExecutableNotFound");
  PYAINAKAN_DECLARE_EXCEPTION (EXECUTABLE_NOT_SUPPORTED, "ExecutableNotSupported");
  PYAINAKAN_DECLARE_EXCEPTION (PROCESS_NOT_FOUND, "ProcessNotFound");
  PYAINAKAN_DECLARE_EXCEPTION (PROCESS_NOT_RESPONDING, "ProcessNotResponding");
  PYAINAKAN_DECLARE_EXCEPTION (INVALID_ARGUMENT, "InvalidArgument");
  PYAINAKAN_DECLARE_EXCEPTION (INVALID_OPERATION, "InvalidOperation");
  PYAINAKAN_DECLARE_EXCEPTION (PERMISSION_DENIED, "PermissionDenied");
  PYAINAKAN_DECLARE_EXCEPTION (ADDRESS_IN_USE, "AddressInUse");
  PYAINAKAN_DECLARE_EXCEPTION (TIMED_OUT, "TimedOut");
  PYAINAKAN_DECLARE_EXCEPTION (NOT_SUPPORTED, "NotSupported");
  PYAINAKAN_DECLARE_EXCEPTION (PROTOCOL, "Protocol");
  PYAINAKAN_DECLARE_EXCEPTION (TRANSPORT, "Transport");

  cancelled_exception = PyErr_NewException ("ainakan.OperationCancelledError", NULL, NULL);
